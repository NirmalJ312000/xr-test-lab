...
/**
 * Short display titles for the validation sequence shown on the report.
 * Presentation only — the checklist definitions in shared/xr-metrics and the
 * exports are untouched.
 */
export const CHECK_DISPLAY = {
  launch: 'Launch / Stability',
...
/** Verdict for a session, with INVALID CAPTURE treated as its own outcome. */
...
export function gradeBadge(letter, score, size) {
  ...
  const tip = score != null ? `Grade ${letter}, ${score} out of 100` : `Grade ${letter}`;
