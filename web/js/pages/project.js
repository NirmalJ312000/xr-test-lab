...
export async function renderProject(main, id) {
  ...
  main.innerHTML = `
    ...
    <div class="page-head">
      <div>
        ...
        <div class="meta">
          ...
          <span>Students <b>${project.students.length}</b></span>
    ...
    <section class="section" aria-labelledby="stu-h">
      <h3 id="stu-h">Students <span class="hint">${project.students.length} on this application</span></h3>
      ${
          ...
          ? `<div class="chip-row">${project.students
              .map(
                (s) => `<a class="entity-chip" href="#/students/${s.id}">
                    ...
                    <span class="muted">${esc(s.studentId ?? 'No ID')} · ${s.sessionCount} test(s)</span>
                  </a>`,
              )
              .join('')}</div>`
  ...
  if (!sessions.length) {
    ...
    histWrap.innerHTML = dataTable({
      ...
      columns: [
        ...
        { key: 'studentName', label: 'Student', sortable: false, render: (r) => esc(r.studentName) },
