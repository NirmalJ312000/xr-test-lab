...
export async function renderStudents(main) {
  main.innerHTML = loadingState({ rows: 8, label: 'Loading students' });
  ...
  main.innerHTML = `
    <div class="page-head">
      <div>
        ...
        <h1>Students</h1>
        <div class="meta"><span class="muted">Everyone with a captured test record</span></div>
      </div>
      <div class="actions"><button type="button" class="btn primary" id="btnNew">New Student</button></div>
    </div>
    ...
    <section class="section">
      <div class="toolbar">
        <div class="field-inline grow">
          ...
          <input class="search" id="fq" type="search" placeholder="Name or student ID…" value="${esc(filters.q)}">
        </div>
...
function draw(main, membership) {
  ...
  const rows = cache.filter(
    (s) =>
      (!q || s.studentName.toLowerCase().includes(q) || (s.studentId ?? '').toLowerCase().includes(q)) &&
  ...
  wrap.innerHTML = dataTable({
    caption: 'Students and their latest test result',
    sort,
    columns: [
      { key: 'studentId', label: 'Student ID', render: (r) => esc(r.studentId ?? '—') },
      {
        key: 'studentName',
        label: 'Student',
        render: (r) =>
...
export async function renderStudent(main, id) {
  destroyAllCharts();
  main.innerHTML = loadingState({ rows: 7, label: 'Loading student' });
  ...
  main.innerHTML = `
    <nav class="crumb" aria-label="Breadcrumb"><a href="#/students">← Students</a></nav>
    ...
    <div class="page-head">
      <div>
        ...
        <div class="meta">
          <span>Student ID <b>${esc(s.studentId ?? '—')}</b></span>
