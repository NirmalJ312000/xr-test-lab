...
export function openStudentForm(student, onDone) {
  ...
  openModal({
    ...
    body: `
      ${field('f_sname', 'Student name', student?.studentName, { placeholder: 'Priya Kannan', required: true })}
      ${field('f_scode', 'Student ID', student?.studentId, { placeholder: '21BCE1042', hint: '(optional)' })}
      ${field('f_email', 'Email', student?.email, { placeholder: 'student@university.edu', hint: '(optional)', type: 'email' })}
      ${
        editing
          ? `<div class="field">
    ...
    footer: `<button type="button" class="btn ghost" data-close>Cancel</button>
             <button type="button" class="btn primary" data-save>${editing ? 'Save Changes' : 'Create Student'}</button>`,
    onMount(root, close) {
      const btn = root.querySelector('[data-save]');
      btn.addEventListener('click', async () => {
        if (!validateFields(root, { f_sname: 'Enter a student name.' })) return;
        const payload = {
          studentName: root.querySelector('#f_sname').value.trim(),
          studentId: root.querySelector('#f_scode').value.trim() || null,
  ...
  removalDialog({
    ...
    onPermanent: () =>
      confirmDanger({
        ...
        lines: [['Student', 1], ...lines],
        confirmLabel: 'Delete Permanently',
        onConfirm: () => api.deleteStudent(student.id, true),
        onDone: (res) => {
          toast(`Student deleted — ${res.sessions} session(s) and ${res.filesRemoved} report file(s) removed`, 'success');
          onDone?.('permanent');
        },
      }),
  });
