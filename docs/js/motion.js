...
export function countUp(scope) {
  ...
  for (const el of els) {
    ...
    // Write the final value FIRST so the DOM is correct even if rAF never fires
    // (backgrounded tab, throttled renderer). The animation then overwrites it.
    el.textContent = final;
    if (reducedMotion() || to === 0) continue;
...
export function drawRings(scope) {
  for (const ring of scope.querySelectorAll('[data-ring]')) {
    ...
    // Start from the empty arc, then hand the real value to the next frame so
    // the CSS transition animates. A timeout backs up rAF in case frames are
    // throttled, so the ring can never be left stuck at zero.
    let drawn = false;
    const paint = () => { if (drawn) return; drawn = true; prog.style.strokeDashoffset = String(target); };
