...
test('Excel export is a single worksheet with the specified columns', async () => {
  ...
  assert.equal(sheet.name, 'XR Test Lab - Overall Report');

  assert.deepEqual(headersOf(sheet), [
    'Project Name', 'Project Platform', 'Target FPS', 'Project Status', 'Total Sessions for Project',
    ...
    'Minimum FPS (info only)', '1% Low FPS (info only)',
    'Launches Without Crash', 'Holds Target Frame Rate', 'Tracking / Input Works',
    'Core Interactions Function', 'Comfortable Locomotion', 'UI Readable in Headset',
    'Spatial Audio Correct', 'Clean Exit / Reset',
    'Checklist Score / 40',
    'Performance Score / 60', 'Final Score / 100', 'Grade', 'Overall Status',
    'Total Bugs', 'Critical Bugs', 'High Bugs', 'Medium Bugs', 'Low Bugs',
  ...
  assert.ok(headersOf(sheet).every((h) => !/\bid\b/i.test(h) || h === 'Student ID'));
});
...
test('Excel export has one row per test session, numbered within its project', async () => {
  ...
  for (const row of rows) {
    const name = String(row.getCell(col('Project Name')).value);
  ...
  for (const [name, projRows] of byProject) {
    ...
    for (const r of projRows) {
      assert.equal(
        r.getCell(col('Total Sessions for Project')).value,
        projRows.length,
...
test('Excel rows carry scores, checklist results and defect counts', async () => {
  ...
  for (const row of rows) {
    ...
    const check = row.getCell(col('Checklist Score / 40')).value;
    ...
    for (const h of ['Launches Without Crash', 'Holds Target Frame Rate', 'Tracking / Input Works',
      'Core Interactions Function', 'Comfortable Locomotion', 'UI Readable in Headset',
      'Spatial Audio Correct', 'Clean Exit / Reset']) {
    ...
    const total = row.getCell(col('Total Bugs')).value;
    const bySeverity = ['Critical Bugs', 'High Bugs', 'Medium Bugs', 'Low Bugs']
      .reduce((a, h) => a + row.getCell(col(h)).value, 0);
    assert.equal(bySeverity, total, 'severity counts must sum to the total');
    assert.ok(row.getCell(col('Open Bugs')).value <= total);
    assert.ok(row.getCell(col('Resolved Bugs')).value <= total);
